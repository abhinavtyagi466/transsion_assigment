...
def _save_interview_record(record: dict):
    ...
    if supabase_url and supabase_key:
        ...
        try:
            ...
            logger.warning(f"Supabase REST save note (saved to local fallback store): {e}")
...
async def interview(body: InterviewRequest):
    """
    Accepts conversation history [{role, text}].
    Sends to Gemini as a professional interviewer following a structured sequence.
    Returns { "response": "interviewer's next question/reply" }
    """
...
async def summarize(body: SummarizeRequest):
    ...
    Generates summary + themes with Gemini 3.1 Flash-Lite.
    Saves history under user_name into Supabase & local DB.
    """
    api_key = _get_api_key()
    genai.configure(api_key=api_key)
